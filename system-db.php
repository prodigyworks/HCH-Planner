		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->refereereportemail = $member['refereereportemail'];
...
function smtpmailer($to, $from, $from_name, $subject, $body, $attachments = array()) { 
	...
	try {
		...
		$mail->SetFrom("info@hchplanner.co.uk", $from_name);
		$mail->IsHTML(true);
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@hchplanner.co.uk', 'Happy Cleaning Homes Planning System', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendInternalRoleMessage($role, $subject, $message, $attachments = array()) {
	$from = "info@hchplanner.co.uk";
	$fromName = "Happy Cleaning Homes Planning System";
...
function sendTeamMessage($id, $subject, $message, $footer = "", $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'confirmation@tclplanner.co.uk', 'Happy Cleaning Homes Planning System', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@hchplanner.co.uk', 'Happy Cleaning Homes Planning System', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
